...
pub fn prepare_ipc_auth_token(token_path: &Path, auth_token: &str) -> Result<()> {
    ...
    let _directories = crate::secure_path::lock_directory_tree(parent, true)?;
    prepare_ipc_auth_token_with(token_path, auth_token, harden_locked_ipc_path)
}
...
where
    F: FnMut(&Path, IpcAclTarget, &File) -> Result<()>,
{
    let parent = token_path
        .parent()
        .context("IPC token path has no parent directory")?;
    ...
    let directory = crate::secure_path::lock_path(parent, true)?;
    harden_path(parent, IpcAclTarget::Directory, &directory).with_context(|| {
        format!(
    ...
    let mut file = crate::secure_path::lock_writable_file(token_path)?;
    harden_path(token_path, IpcAclTarget::TokenFile, &file).with_context(|| {
        format!(
            "Failed to harden IPC token permissions at {}",
            token_path.display()
        )
    })?;
    file.set_len(0).context("Failed to truncate IPC token")?;
...
pub fn harden_ipc_permissions(path: &Path, target: IpcAclTarget) -> Result<()> {
    let parent = path.parent().context("IPC path has no parent directory")?;
    let _directories = crate::secure_path::lock_directory_tree(parent, false)?;
    let guard = crate::secure_path::lock_path(path, target == IpcAclTarget::Directory)?;
    harden_locked_ipc_path(path, target, &guard)
}

fn harden_locked_ipc_path(path: &Path, target: IpcAclTarget, guard: &File) -> Result<()> {
    crate::secure_path::ensure_trusted_owner(guard)
        .with_context(|| format!("Unsafe IPC path {}", path.display()))?;
    let user_sid = active_console_user_sid().filter(|sid| is_valid_sid(sid));
    if user_sid.is_none() {
...
/// `Access` section, after the caller has pinned the path and verified that
/// its owner is a trusted service/administrator identity.
///
