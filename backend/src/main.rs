...
async fn main() -> Result<()> {
    ...
    // Explicitly disabling MTU discovery. 
    // This ensures Quinn maintains the required MTU for inner 1280-byte packets.
    ...
    // Allow IP fragmentation on the server side as well. 
    // We use libc directly to avoid socket2 API version issues.
    unsafe {
