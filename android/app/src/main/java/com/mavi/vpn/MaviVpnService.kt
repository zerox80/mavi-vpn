...
class MaviVpnService : VpnService() {
    ...
    private external fun init(service: MaviVpnService, token: String, endpoint: String, certPin: String, transportMode: Int): Long
        ...
        thread = Thread {
            ...
            while (isRunning) {
                try {
                    ...
                    // 1. Init / Handshake
                    val transportMode = getSharedPreferences("MaviVPN", Context.MODE_PRIVATE)
                        .getInt("saved_transport_mode", 0) // 0=QUIC, 1=HTTP/3, 2=HTTP/2
                    val handle = init(this, token, "$ip:$port", certPin, transportMode)
                    if (handle == 0L) {
    ...
    override fun onDestroy() {
        super.onDestroy()
        // Safety net: release WakeLock if stopVpn() was never called
        try {
            if (wakeLock?.isHeld == true) {
                wakeLock?.release()
            }
    ...
    private fun netmaskToPrefixLength(netmask: String): Int {
        return try {
            ...
            // Validate contiguous bits: a valid netmask has all 1s followed by all 0s
