...
fn token_file_sddl_grants_only_system_admins_and_user_read() {
    ...
    assert_eq!(sddl, "D:P(A;;FA;;;SY)(A;;FA;;;BA)(A;;FR;;;S-1-5-21-1000)");
}
...
fn acl_script_replaces_dacl_atomically_and_preserves_owner() {
    ...
    assert!(script.contains(
        "[System.IO.File]::SetAccessControl('C:\\ProgramData\\mavi-vpn\\ipc.token', $acl)"
    ));
}
...
fn acl_script_uses_directory_apis_for_directory_target() {
    ...
    assert!(script
        .contains("[System.IO.Directory]::SetAccessControl('C:\\ProgramData\\mavi-vpn', $acl)"));
...
fn hardening_script_removes_stale_aces() {
    ...
    assert!(
        is_valid_sid(&current_user_sid),
        "got SID {current_user_sid:?}"
    );
    ...
    let allow_with = |sid: &str, right: &str| {
        aces.lines().any(|line| {
            line.starts_with("Allow:") && line.contains(&format!(":{sid}:")) && line.contains(right)
        })
    };
