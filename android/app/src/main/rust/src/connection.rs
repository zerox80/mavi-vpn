use bytes::Bytes;
use log::info;
use shared::{compute_quic_mtu_config, control, looks_like_html_response, ControlMessage};
...
#[cfg(test)]
use http3::is_camouflage_h3_response;
pub use http3::H3SessionGuard;
...
pub async fn connect_and_handshake(
    ...
    let (config, h3_guard) = if effective_http3_framing {
        let (cfg, guard) = http3::connect_and_handshake(connection.clone(), token).await?;
        (cfg, Some(guard))
    } else {
