//! HTTP/2 CONNECT-IP transport (RFC 9484 carried by RFC 9297 capsules).

use super::cert::PinnedServerVerifier;
...
impl Http2Session {
    pub(in crate::vpn_core) const fn remote_addr(&self) -> std::net::SocketAddr {
        self.remote_addr
    }
    ...
    pub(in crate::vpn_core) async fn reauthenticate(&self, token: &str) -> Result<bool> {
